...
def parse_datetime(value, label):
    ...
    try:
        ...
        raise RuntimeError(
            f"ERROR: {label} is not a valid ISO timestamp: {value}"
        ) from exc
...
def parse_date(value, label):
    ...
    try:
        ...
        raise RuntimeError(
            f"ERROR: invalid date in {label}: {value}"
        ) from exc
...
def validate_required_fields(records, dataset_name):
    for index, opportunity in enumerate(records, start=1):
        if not isinstance(opportunity, dict):
            raise RuntimeError(
                f"ERROR: {dataset_name} record {index} is not an object."
            )
        ...
        if missing:
            raise RuntimeError(
                f"ERROR: {dataset_name} record {index} "
                f"({opportunity.get('id', '?')}) is missing: {', '.join(missing)}"
            )

        if not str(opportunity.get("id", "")).strip():
            raise RuntimeError(
                f"ERROR: {dataset_name} contains a record with an empty id."
            )

        if not isinstance(opportunity.get("eligible_countries"), list):
            raise RuntimeError(
                f"ERROR: {dataset_name} record {opportunity.get('id')} "
                "has invalid eligible_countries."
            )
...
def expiry_date_for(opportunity):
    deadline = parse_date(
        ...
        f"deadline for {opportunity.get('id')}",
    )
    end_date = parse_date(
        opportunity.get("end_date"),
        f"end_date for {opportunity.get('id')}",
    )
...
def validate_active(records, today):
    ...
    for opportunity in records:
        ...
        if opid in seen:
            raise RuntimeError(
                f"ERROR: duplicate active opportunity id: {opid}"
            )
        seen.add(opid)

        deadline = parse_date(
            opportunity.get("deadline"),
            f"deadline for active opportunity {opid}",
        )
        end_date = parse_date(
            opportunity.get("end_date"),
            f"end_date for active opportunity {opid}",
        )

        if deadline is not None and deadline < today:
            raise RuntimeError(
                f"ERROR: expired deadline remains active: {opid} ({deadline})"
            )

        if deadline is None and end_date is not None and end_date < today:
            raise RuntimeError(
                f"ERROR: finished no-deadline activity remains active: {opid} ({end_date})"
            )
...
def validate_expired(records, today):
    ...
    for opportunity in records:
        ...
        if opid in seen:
            raise RuntimeError(
                f"ERROR: duplicate recently expired opportunity id: {opid}"
            )
        seen.add(opid)
        ...
        if expiry is None:
            raise RuntimeError(
                f"ERROR: recently expired opportunity has no deadline/end date: {opid}"
            )

        if expiry > today:
            raise RuntimeError(
                f"ERROR: future-dated record appears in recently expired: {opid} ({expiry})"
            )

        if expiry < cutoff:
            raise RuntimeError(
                f"ERROR: expired opportunity older than {RECENTLY_EXPIRED_DAYS} days remains published: "
                f"{opid} ({expiry})"
            )
...
def validate():
    active_payload = load_object(
        OPPORTUNITIES_FILE,
        "data/opportunities.json",
    )
    expired_payload = load_object(
        EXPIRED_FILE,
        "data/expired.json",
    )
    ...
    if not isinstance(active, list) or not active:
        raise RuntimeError(
            "ERROR: active opportunity dataset is empty or invalid."
        )

    if not isinstance(expired, list):
        raise RuntimeError(
            "ERROR: recently expired dataset is invalid."
        )

    if active_payload.get("count") != len(active):
        raise RuntimeError(
            "ERROR: active count metadata does not match records."
        )

    if expired_payload.get("count") != len(expired):
        raise RuntimeError(
            "ERROR: expired count metadata does not match records."
        )

    generated_at = parse_datetime(
        active_payload.get("generated_at"),
        "data/opportunities.json generated_at",
    )
    ...
    if age < timedelta(seconds=0):
        raise RuntimeError(
            "ERROR: dataset generated_at is in the future."
        )

    if age > MAX_DATASET_AGE:
        raise RuntimeError(
            f"ERROR: published dataset is stale ({age})."
        )
    ...
    validate_active(active, today)
    validate_expired(expired, today)
    ...
    if not isinstance(new_ids, list):
        raise RuntimeError(
            "ERROR: new_opportunity_ids must be a list."
        )

    normalized_new_ids = [str(value) for value in new_ids]
    if len(normalized_new_ids) != len(set(normalized_new_ids)):
        raise RuntimeError(
            "ERROR: new_opportunity_ids contains duplicates."
        )

    missing_new_ids = [
        opid
        for opid in normalized_new_ids
        if opid not in active_ids
    ]
    ...
    if not isinstance(active_payload.get("scan_complete"), bool):
        raise RuntimeError(
            "ERROR: scan_complete must be boolean."
        )
