...
def main():
    ...
    if MARKER not in text:
        ...
        if start < 0 or end < 0:
            raise SystemExit("ERROR: could not locate fetch_current_opportunities boundaries; no source change made.")
        text = text[:start] + "\n".join(NEW_FUNCTION_LINES) + text[end:]
    ...
    if replacements != 1:
        raise SystemExit("ERROR: could not safely set API_PAGE_SIZE to 1000; no source change made.")
    if MARKER not in text:
        raise SystemExit("ERROR: pagination marker was not written; no source change made.")
